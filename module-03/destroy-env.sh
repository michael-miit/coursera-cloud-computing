...
TARGETARN=$(aws elbv2 describe-target-groups --names mm-tg --query 'TargetGroups[*].TargetGroupArn' --output text)
echo $TARGETARN
...
ELBARN=$(aws elbv2 describe-load-balancers --names mm-elb --query 'LoadBalancers[*].LoadBalancerArn' --output text)
echo $ELBARN
